...
double height_step = 1.0;//飞行步进高度
double takeoff_height = 0.5 * height_step;//命令下发的飞行高度，初始高度等于步进高度
double pos_hold_height = takeoff_height;//定高高度
double length = 4.0;//货架长度（即左右飞行距离）
ros::Time takeoff_start_time;//开始起飞时间
int cmd_received_count = 0;//接收指令的数量
    ...
    while(ros::ok())
    {
        ...
        switch (uav_state)
        {
            case UAV_READY://准备状态
            {
                ...
                if(current_state.mode == "OFFBOARD" && current_state.armed)//切offboard转入起飞状态
                {
                    ...
                    if((uavCurrentLocalPose.pose.position.z>takeoff_height-0.1)&&//到达起飞高度且保证至少5s的起飞时间
                       (ros::Time::now() - takeoff_start_time > ros::Duration(5.0)))
                    {
            ...
            case UAV_POS_HOLD://定点模式
            {
                ...
                uav_poshold_tar.position.x = uavCurrentLocalPose.pose.position.x;
                uav_poshold_tar.position.y = uavCurrentLocalPose.pose.position.y;
                uav_poshold_tar.position.z = uavCurrentLocalPose.pose.position.z;
                uav_poshold_tar.yaw = uavYawENU;
                local_raw_tar_pub.publish(uav_poshold_tar);
                if((uavCurrentLocalPose.pose.position.z > 0.5-0.1)&&//takeoff_height=0.5
                    (uavCurrentLocalPose.pose.position.x > 0.0-0.1)&&
                    (ros::Time::now() - uav_poshold_tar.header.stamp > ros::Duration(3.0)))
                {
                    uav_state = UAV_FLY_LEFT;
                    std::cout << "Enter in UAV_FLY_LEFT mode!" << std::endl;
                }
                else if((uavCurrentLocalPose.pose.position.z > 0.5-0.1)&&//takeoff_height=0.5
                        (uavCurrentLocalPose.pose.position.x < length+0.1)&&
                        (ros::Time::now() - uav_poshold_tar.header.stamp > ros::Duration(3.0)))
                {
                    uav_state = UAV_TAKEOFF;
                    std::cout << "Enter in UAV_TAKEOFF mode!" << std::endl;
                }
                else if((uavCurrentLocalPose.pose.position.z > 1.5-0.1)&&//takeoff_height=1.5
                        (uavCurrentLocalPose.pose.position.x < length+0.1)&&
                        (ros::Time::now() - uav_poshold_tar.header.stamp > ros::Duration(3.0)))
                {
                ...
                else if((uavCurrentLocalPose.pose.position.z > 2.5-0.1)&&//takeoff_height=2.5
                        (uavCurrentLocalPose.pose.position.x > 0.0-0.1)&&
                        (ros::Time::now() - uav_poshold_tar.header.stamp > ros::Duration(3.0)))
                {
                    uav_state = UAV_FLY_LEFT;
            ...
            case UAV_TAKEOFF://起飞升高状态
            {
                uav_poshold_tar.header.stamp = ros::Time::now();
                takeoff_height = height_step + uavCurrentLocalPose.pose.position.z;
                uav_poshold_tar.position.x = uavCurrentLocalPose.pose.position.x;
                uav_poshold_tar.position.y = uavCurrentLocalPose.pose.position.y;
                uav_poshold_tar.position.z = takeoff_height;
                uav_poshold_tar.yaw = uavYawENU;
